...
def compute_tfidf_matches(df_reference, df_target, synonym_map=DEFAULT_SYNONYM_MAP):
    df_reference = df_reference[df_reference["snowflake_heading"].notna()].reset_index(drop=True)
    df_target = df_target[df_target["heading_0"].notna()].reset_index(drop=True)
    df_reference["norm_text"] = df_reference["snowflake_heading"].apply(enhanced_normalize)
    ...
    for sim_row in similarity_matrix:
        ...
        matched_qs.append(df_reference.iloc[best_idx]["snowflake_heading"] if best_idx is not None else None)
...
def compute_semantic_matches(df_reference, df_target):
    try:
        ...
        emb_ref = model.encode(df_reference["snowflake_heading"].tolist(), convert_to_tensor=True)
...
def finalize_matches(df_target, df_reference):
    ...
    df_target["Change_UID"] = df_target["Final_UID"].apply(
        lambda x: f"{x} - {df_reference[df_reference['UID'] == x]['snowflake_heading'].iloc[0]}" if pd.notnull(x) and x in df_reference["UID"].values else None
    )
...
try:
    ...
    if st.session_state.question_bank is None:
        try:
            question_bank = run_snowflake_reference_query()
            question_bank = question_bank.rename(columns={"HEADING_0": "snowflake_heading"})
            st.session_state.question_bank = question_bank
        except Exception:
            st.warning("Failed to load question bank. Standardization checks disabled.")
            st.session_state.question_bank = pd.DataFrame(columns=["snowflake_heading", "UID"])
...
with tab1:
    ...
    if st.button("View Question Bank"):
        ...
        filtered_bank = st.session_state.question_bank[
            st.session_state.question_bank["snowflake_heading"].str.contains(search_query, case=False, na=False) |
        ...
        st.dataframe(filtered_bank.rename(columns={"snowflake_heading": "Question/Choice (Snowflake)", "UID": "UID"}))
...
with tab2:
    if st.session_state.df_target is not None and not st.session_state.df_target.empty:
        ...
        uid_options = [None] + [f"{row['UID']} - {row['snowflake_heading']}" for _, row in st.session_state.question_bank.iterrows()]
            ...
            with st.form("survey_creation_form"):
                ...
                if st.button("Validate Against Question Bank"):
                    non_standard = edited_df[
                        ~edited_df["heading_0"].str.split(" - ", n=1, expand=True)[0].isin(st.session_state.question_bank["snowflake_heading"])
                    ]
